package com.intergration.paymentcenter.unionpaypayment.controller;

import com.intergration.paymentcenter.unionpaypayment.sdk.AcpService;
import com.intergration.paymentcenter.unionpaypayment.sdk.LogUtil;
...
public class BusinessController {
    ...
    @RequestMapping(value = "/jumpToUnionpay", method = RequestMethod.POST)
    @ApiOperation(value = "跳转到银联支付页面", tags = {"payment: 银联接口"}, notes = "{\"amount\":\"100\"}")
    public void jumpToUnionpay(@RequestBody @ApiParam(name="付款金额",value="{\"amount\":\"100\"}",required=true) Map<String, String> paraMap, HttpServletRequest request, HttpServletResponse resp) throws IOException {

        resp.setContentType("text/html; charset="+ DemoBase.encoding);

        //前台页面传过来的
        String merId = "777290058110048";//商户号
        String txnAmt = paraMap.getOrDefault("amount", "0");//订单金额

        String orderId = "20201223115048";//订单号
        String txnTime = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date().getTime());//订单时间
